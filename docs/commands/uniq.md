...
Deseret, and Fullwidth Latin uppercase pairs without case significance. `-u` emits only unrepeated groups. `-f fields` skips leading
...
Field skipping treats Unicode whitespace under the project UTF-8 text policy as field separators. Character skipping
...
## Locale Behavior
Filtered data is not localized. Help and diagnostics are localized.
...
full arbitrary POSIX locale semantics. Case folding is deterministic and locale-independent.
...
## Known Limitations
Full arbitrary POSIX locale collation and case mapping are not implemented; the V1 behavior is the documented
deterministic UTF-8 policy.
