...
- Common-runner rewrites of `awk`, `grep`, or `sed`; they are integrated as
  `tools/awk`, `tools/grep`, and `tools/sed` around process-facing wrappers
  backed by `awklib`, `greplib`, and `sedlib`.
