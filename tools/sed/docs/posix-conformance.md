...
Test identifiers name cases in the original sed suite and the integrated
posix_tools smoke checks.
...
because anything is known to be missing.
...
3 is implementation-defined and documented in `docs/command-line.md`.
...
identifier to the tracked posix_tools requirements; and writing a test that
asserts the divergence, so that fixing the cause fails the test and forces the
entry to be closed.
