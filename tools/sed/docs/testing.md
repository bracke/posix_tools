...
The original standalone suite lived in the `sed_tests` child crate and ran
through AUnit. In posix_tools, sed is additionally covered by integrated
metadata checks and command smoke tests.
...
`docs/posix-conformance.md` can cite the case that pins each claim.
...
```sh
cd tools/sed && alr build
tests/bin/posix_tools_tests check
```
