...
package body Posix_Tools.Commands.Expanded is
   ...
   is
      ...
      function Rounded_Units (Bytes : Long_Long_Integer) return Long_Long_Integer is
      begin
         if Bytes <= 0 then
            return 0;
         else
      ...
      begin
         if not Context.Output_Failed then
            Context.Put_Line (Long_Image (Units) & Character'Val (9) & Path);
         end if;
      end Print_Usage;
            ...
            if Arg = "--" then
               ...
               for I in Arg'First + 1 .. Arg'Last loop
                  case Arg (I) is
                     ...
                     when others =>
                        Posix_Tools.Commands.Helpers.Usage_Error (Context, Result, "unknown option '-" & Arg (I) & "'");
                        return;
                  end case;
               end loop;
               First := First + 1;
   ...
   is
      ...
      function Is_False (Value : Expr_Value) return Boolean is
         Text : constant String := To_String (Value.Text);
      begin
         return Text = "" or else Text = "0";
      end Is_False;
      ...
      begin
         ...
         Value := (if Negative then -Acc else Acc);
         return True;
      end Parse_Integer;
      ...
      begin
         if not Failed then
            Posix_Tools.Commands.Helpers.Usage_Error (Context, Result, Message);
            Failed := True;
         end if;
      end Fail;
      ...
      begin
         return Position <= Context.Argument_Count and then Context.Argument (Position) = Token;
      end Have;
      ...
      begin
         if Failed then
            return Make ("");
         elsif Position > Context.Argument_Count then
            Fail ("missing expression operand");
            return Make ("");
         end if;

         Token := To_Unbounded_String (Context.Argument (Position));
         Position := Position + 1;
         ...
         if To_String (Token) = "(" then
            ...
            Fail ("unexpected ')' in expression");
            return Make ("");
         elsif To_String (Token) = "length" then
            declare
               Value : constant Expr_Value := Eval_Primary;
            begin
               return Make (Image (Long_Long_Integer (Length (Value.Text))));
            end;
            ...
            begin
               for I in Source'Range loop
                  for Ch of Chars loop
                     if Source (I) = Ch then
                        return Make (Image (Long_Long_Integer (I - Source'First + 1)));
                     end if;
                  end loop;
               end loop;
               return Make ("0");
            ...
            begin
               if not Parse_Integer (Start_Text, Start_Num) or else not Parse_Integer (Len_Text, Len_Num) then
                  Fail ("non-numeric substr operand");
                  return Make ("");
      ...
      begin
         while not Failed and then Position <= Context.Argument_Count
           and then Context.Argument (Position) in "*" | "/" | "%"
         loop
            declare
               Op    : constant String := Context.Argument (Position);
               Right : Expr_Value;
            ...
            begin
               ...
               then
                  Fail ("non-numeric arithmetic operand");
                  return Make ("");
               ...
               begin
                  if Op = "*" then
                     Left := Make (Image (L_Int * R_Int));
                  elsif Op = "/" then
                     Left := Make (Image (L_Int / R_Int));
                  else
                     Left := Make (Image (L_Int mod R_Int));
                  end if;
               exception
                  when Constraint_Error =>
      ...
      begin
         while not Failed and then Consume ("&") loop
            ...
            begin
               Left := (if Is_False (Left) or else Is_False (Right) then Make ("0") else Left);
            end;
         end loop;
         return Left;
      end Eval_And;
      ...
      begin
         while not Failed and then Consume ("|") loop
            ...
            begin
               Left := (if Is_False (Left) then Right else Left);
            end;
         end loop;
         return Left;
      end Eval_Or;
   ...
   begin
      if Context.Argument_Count = 0 then
         Posix_Tools.Commands.Helpers.Usage_Error (Context, Result, "missing expression");
         return;
      end if;
      ...
      if Failed then
         ...
         Posix_Tools.Commands.Helpers.Usage_Error
           (Context, Result, "extra operand '" & Context.Argument (Position) & "'");
         return;
      end if;

      Context.Put_Line (To_String (Value.Text));
      Result.Status :=
        (if Context.Output_Failed then Posix_Tools.Exit_Status.Operational_Failure
         elsif Is_False (Value) then Posix_Tools.Exit_Status.Operational_Failure
         else Posix_Tools.Exit_Status.Success);
   end Run_Expr;
